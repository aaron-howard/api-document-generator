...
class HTMLGenerator {
    constructor() {
        ...
        this.defaultTemplate = 'default-html';
    ...
    async renderHTML(template, context, options = {}) {
    ...
    generateEndpointsHTML(endpoints, options = {}) {
        if (!endpoints || endpoints.length === 0) {
    ...
    generateEndpointHTML(endpoint, options = {}) {
        const id = this.generateId(`${endpoint.method}-${endpoint.path}`);
    ...
    generateWarnings(_request, content) {
        const warnings = [];
        // Check content size
