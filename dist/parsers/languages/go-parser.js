    ...
    async loadSource(request) {
        switch (request.source) {
            ...
            case 'content':
                return request.path;
            default:
                throw new parser_service_1.ParseError(`Unsupported source type for Go: ${request.source}`, 'UNSUPPORTED_SOURCE');
        }
    }
...
`;
    ...
    extractNameFromComment(comment, type) {
    ...
    async parseDocComments(comments) {
    ...
    isSectionHeader(line) {
        ...
        return sectionHeaders.some(header => line.toLowerCase().includes(header.toLowerCase()));
    }
    getSectionName(line) {
        if (line.toLowerCase().includes('route'))
            return 'route';
        ...
        if (line.toLowerCase().includes('request body'))
            return 'request_body';
    ...
    async convertToAST(parsedDocs, request) {
        ...
        const spec = {
            ...
            format: api_spec_1.ApiSpecFormat.GO_GODOC,
            version: '1.0.0',
            metadata: {
                title: 'Go API',
    ...
    inferRouteFromFunction(functionName) {
    ...
    // Validation methods
    validateGodocFormat(ast, violations) {
        if (!ast.metadata) {
            violations.push({
                rule: 'godoc-format',
    ...
    validateStructDocumentation(ast, violations) {
...
exports.GoDocParser = GoDocParser;
exports.default = GoDocParser;
