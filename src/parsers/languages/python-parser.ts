    ...
    switch (request.source) {
      ...
      default:
        throw new ParseError(
          `Unsupported source type for Python: ${request.source}`,
          'UNSUPPORTED_SOURCE'
        );
    }
  }
...
`;
  ...
  private extractDocstrings(source: string): any[] {
  ...
  private async parseDocstrings(docstrings: any[]): Promise<any[]> {
  ...
  private parseDocstring(docstring: any): any | null {
  ...
  private detectDocstringFormat(content: string): string {
  ...
  private parseGoogleDocstring(content: string, parsed: any): void {
  ...
  private parseNumpyDocstring(content: string, parsed: any): void {
    // Simplified NumPy format parsing
    this.parseGoogleDocstring(content, parsed);
  }

  private parseSphinxDocstring(content: string, parsed: any): void {
    const lines = content.split('\n');
    let currentSection = 'description';
    let sectionContent: string[] = [];
  ...
  private parseSimpleDocstring(content: string, parsed: any): void {
  ...
  private parseParameterSection(lines: string[]): any[] {
  ...
  private parseReturnSection(lines: string[]): any {
  ...
  private parseRaisesSection(lines: string[]): any[] {
    const raises: any[] = [];
  ...
  private async convertToAST(parsedDocs: any[], request: ParseRequest): Promise<StandardizedAST> {
    ...
    const spec: ApiSpecification = {
      ...
      format: ApiSpecFormat.PYTHON_DOCSTRING,
      version: '1.0.0',
      metadata: {
        title: 'Python API',
  ...
  private extractRouteInfo(doc: any): { method: HttpMethod; path: string } | null {
  ...
  private convertPythonType(pythonType: string): string {
  ...
  private mapExceptionToStatus(exception: string): string {
  ...
  private validateDocstringFormat(ast: any, violations: ValidationResponse['violations']): void {
    if (!ast.metadata) {
      violations!.push({
  ...
  private validateTypeAnnotations(ast: any, violations: ValidationResponse['violations']): void {
